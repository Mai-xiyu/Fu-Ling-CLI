# 符灵扩展指南

## 🎯 概述

符灵设计为可扩展的。你可以：
- 创建自定义命令
- 添加新的AI提供商
- 开发插件
- 自定义主题
- 集成其他工具

## 🚀 快速扩展示例

### 1. 创建自定义命令

**文件**: `~/.config/fuling/plugins/my_commands.py`
```python
import click
from fuling import format_text

@click.command()
@click.argument('name')
def hello(name):
    """打招呼"""
    click.echo(format_text(f"你好, {name}!", "success"))

# 注册到符灵
def register_commands(cli):
    cli.add_command(hello)
```

**使用**:
```bash
# 添加到config.yaml
plugins:
  - ~/.config/fuling/plugins/my_commands.py

# 使用命令
fl hello 世界
```

### 2. 添加简单AI提供商

**文件**: `~/.config/fuling/providers/custom_provider.py`
```python
from fuling.fuling_ai import AIProvider
from typing import Dict, Any, List, Optional

class CustomProvider(AIProvider):
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.api_url = config.get('api_url', '')
    
    def chat_completion(self, messages: List[Dict], **kwargs) -> str:
        # 你的自定义AI集成
        return "来自自定义提供商的响应"
    
    def explain_command(self, command: str, context: Optional[str] = None) -> str:
        return f"自定义解释: {command}"
```

## 📦 插件系统

### 插件结构
```
my_fuling_plugin/
├── __init__.py
├── commands.py      # 自定义命令
├── providers.py     # AI提供商
├── themes.py        # 自定义主题
└── config.yaml      # 插件配置
```

### 插件注册
```python
# 在插件的 __init__.py 中
from .commands import register_commands
from .providers import CustomProvider

__all__ = ['register_commands', 'CustomProvider']
```

### 配置
```yaml
# config.yaml
plugins:
  - /path/to/plugin1
  - /path/to/plugin2
  
plugin_settings:
  plugin1:
    enabled: true
    config_key: value
```

## 🤖 自定义AI提供商

### 提供商接口
```python
from abc import ABC, abstractmethod

class BaseAIProvider(ABC):
    @abstractmethod
    def chat_completion(self, messages: List[Dict], **kwargs) -> str:
        pass
    
    @abstractmethod
    def explain_command(self, command: str, context: str = None) -> str:
        pass
    
    @abstractmethod
    def suggest_commands(self, context: str = None) -> List[Dict]:
        pass
```

### 示例：本地文件提供商
```python
class FileProvider(AIProvider):
    def __init__(self, config):
        super().__init__(config)
        self.knowledge_file = config.get('knowledge_file', '')
    
    def explain_command(self, command, context=None):
        # 从本地知识库读取
        with open(self.knowledge_file, 'r') as f:
            knowledge = json.load(f)
        return knowledge.get(command, "知识库中未找到此命令")
```

## 🎨 自定义主题

### 主题结构
```python
from fuling.fuling_theme import Theme

class MyCustomTheme(Theme):
    def get_banner(self):
        return """
    ╔══════════════════════════╗
    ║     我的自定义主题       ║
    ╚══════════════════════════╝
    """
    
    def format_text(self, text, style=None):
        # 自定义格式化逻辑
        symbols = {
            'success': '🎉',
            'error': '💥',
            'info': '💡',
        }
        symbol = symbols.get(style, '•')
        return f"{symbol} {text}"
```

### 注册主题
```python
# 添加到config.yaml
theme:
  name: "mycustom"
  # 主题配置
```

## 🔌 集成示例

### 1. 与Git集成
```python
@click.command()
@click.option('--message', '-m', required=True)
def git_commit(message):
    """使用AI生成提交信息的Git提交"""
    from fuling import chat_completion
    
    # 使用AI生成提交信息
    messages = [
        {"role": "system", "content": "生成git提交信息"},
        {"role": "user", "content": f"更改: {message}"}
    ]
    
    ai_message = chat_completion(messages)
    os.system(f'git commit -m "{ai_message}"')
```

### 2. 系统监控插件
```python
@click.command()
def system_health():
    """检查系统健康状态"""
    import psutil
    
    cpu = psutil.cpu_percent()
    memory = psutil.virtual_memory().percent
    disk = psutil.disk_usage('/').percent
    
    click.echo(f"CPU: {cpu}%")
    click.echo(f"内存: {memory}%")
    click.echo(f"磁盘: {disk}%")
```

### 3. Docker助手
```python
@click.command()
@click.argument('service')
def docker_logs(service):
    """显示并过滤Docker日志"""
    import subprocess
    
    # 获取日志
    result = subprocess.run(
        ['docker', 'logs', '--tail', '100', service],
        capture_output=True,
        text=True
    )
    
    # 使用AI分析日志
    from fuling import explain_command
    analysis = explain_command(f"docker日志分析: {result.stdout[:500]}")
    
    click.echo(analysis)
```

## 🧪 测试扩展

### 单元测试
```python
import pytest
from my_plugin.commands import hello

def test_hello_command():
    from click.testing import CliRunner
    
    runner = CliRunner()
    result = runner.invoke(hello, ['世界'])
    
    assert result.exit_code == 0
    assert '你好' in result.output
```

### 集成测试
```python
def test_plugin_integration():
    # 测试插件是否正确加载
    from fuling import get_config
    config = get_config()
    
    assert 'plugins' in config
    assert 'my_plugin' in config['plugins']
```

## 📁 扩展项目结构

### 推荐结构
```
fuling-extension/
├── pyproject.toml          # 包配置
├── README.md              # 文档
├── src/
│   └── fuling_extension/
│       ├── __init__.py
│       ├── commands.py
│       ├── providers.py
│       └── themes.py
├── tests/                 # 测试
└── examples/             # 使用示例
```

### 包配置
```toml
# pyproject.toml
[project]
name = "fuling-extension-example"
version = "0.1.0"
description = "符灵扩展示例"

[project.entry-points."fuling.plugins"]
example = "fuling_extension:register_plugin"
```

## 🚀 发布扩展

### 1. 打包扩展
```bash
# 构建包
python -m build

# 上传到PyPI
twine upload dist/*
```

### 2. 在GitHub分享
```bash
# 创建仓库
git init
git add .
git commit -m "初始提交"
git remote add origin https://github.com/username/fuling-extension.git
git push -u origin main
```

### 3. 社区分享
- 添加到符灵插件注册表
- 在GitHub讨论区分享
- 创建文档
- 提供示例

## 🔧 高级主题

### 1. 配置管理
```python
# 动态配置
from fuling.fuling_core import get_config, config

def update_config():
    current = get_config()
    current['custom_key'] = 'value'
    config.save_config(current)
```

### 2. 事件系统
```python
# 订阅事件
def on_command_executed(command, result):
    print(f"命令执行: {command}")
    # 记录到文件、发送通知等

# 注册事件处理器
register_event_handler('command_executed', on_command_executed)
```

### 3. 缓存
```python
from functools import lru_cache

@lru_cache(maxsize=100)
def cached_explanation(command):
    # 昂贵操作
    return expensive_ai_call(command)
```

## 📚 资源

### 官方文档
- [符灵API参考](API_REFERENCE.md)
- [插件开发指南](PLUGIN_DEVELOPMENT.md)
- [主题定制](THEME_CUSTOMIZATION.md)

### 社区资源
- [GitHub仓库](https://github.com/mai-xiyu/Fu-Ling-CLI)
- [插件示例](https://github.com/mai-xiyu/Fu-Ling-CLI/tree/main/examples)
- [讨论论坛](https://github.com/mai-xiyu/Fu-Ling-CLI/discussions)

### 工具和库
- Click: CLI框架
- PyYAML: 配置管理
- Requests: HTTP客户端
- Pytest: 测试框架

---

**立即开始扩展符灵！查看examples目录获取更多灵感。** 🚀